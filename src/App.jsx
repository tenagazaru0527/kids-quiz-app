...
// ============================================================
// クイズデータは GAS(Google Drive の quizzes/ジャンル名/xxx.json)から
// useQuizData フックが取得する。データ形式は以下:
// { title, category, emoji, difficulty(1-3), createdAt,
...
function LoadingScreen() {
  return (
    <div style={{ ...S.root, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20 }}>
      ...
      <div style={{ fontSize: 22, fontWeight: 700 }}>クイズをよみこみ中…</div>
      <div style={{ color: "#8B8DBB", fontSize: 14 }}>ちょっとまってね</div>
    </div>
  );
}
...
function ErrorScreen({ error, onRetry }) {
  return (
    <div style={{ ...S.root, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20, padding: 24, boxSizing: "border-box" }}>
      <div style={{ fontSize: 56 }}>😢</div>
      <div style={{ fontSize: 22, fontWeight: 700 }}>クイズがよみこめなかったよ</div>
      <div style={{ color: "#8B8DBB", fontSize: 15, textAlign: "center", lineHeight: 1.7 }}>
        インターネットにつながっているか確認してね。
        <br />
        なおらないときは、おうちの人を呼ぼう!
      </div>
      <button style={{ ...S.bigBtn, background: "#4DA3FF", color: "#fff" }} onClick={onRetry}>
        もういちど よみこむ
      </button>
      {error && <div style={{ color: "#565880", fontSize: 12, marginTop: 8 }}>詳細: {error}</div>}
    </div>
  );
}
...
function QuizSelect({ quizzes, mode, onPick, onBack }) {
  ...
  const list = quizzes.map((quiz, index) => ({ quiz, index }))
